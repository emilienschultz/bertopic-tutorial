...
![**XXX** [^18]](./assets/tuto_2d_plot.png){#fig-2d-plot}
...
![**XXX**](./assets/tuto_hierarchical.png){#fig-hierarchical-plot}
...
To aggregate topics, the algorithm proposed is to use the topic embeddings[^23], compute the cosine similarity matrix and use the agglomerative clustering algorith described [here](XXX) to aggregate the topics. Once performed, all the documents are moved into a new group and keywords are re-generated.
<br/>
...
: Table X: **XXX** {tbl-colwidths="[10, 20, 70]" #tbl-topics-tuned}

![**XXX**](./assets/2D_general.png){#fig-2D-map-tuned}
...
As depicted in @sec-tune-parameters, exploring the 2D map is a good way to explore the embedding space and tell if groups are made out of similar documents. It's even better if you have additional tags that can make the exploration even faster.
...
_For more information on the content of what's saved and the size of files._
...
[^24]: There is a small difference in the computation of the graph and the reduce outlier that we do not clearly understand. [See issue #XXX.](XXX)